...
def main():
    ...
    if os.path.isdir(MODEL_PATH) and RESUME:
        model.load_state_dict(torch.load(MODEL_PATH + '/model5.pth'))
    ...
    for epoch in range(EPOCHS):
        ...
        for i, data in progress_bar:
            ...
            # --- Update tqdm display with current losses ---
        ...
        print(f"\n[Epoch {epoch+1}] Time: {end_time - start_time:.2f}s, Losses: {losses}")
        ...
        if epoch % SAVE_MODEL_HISTORY == 0:
            torch.save(model.state_dict(), MODEL_PATH + f'/model{epoch}.pth')
