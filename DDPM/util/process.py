...
import numpy as np
import torch
from util.image_utils import *,os
import torch
...
def fg_by_cc(thres, max_num):
    ...
    def func(seg_map):
        ...
        num_labels, label_img = connectedComponents(seg_mask, connectivity=4)
...
def word_segment(path, output_folder, model_path):
    ...
    if f < 1:
        orig = resize(orig, dsize=None, fx=f, fy=f)
    ...
    for idx,bb in enumerate(clustered_aabbs):
        ...
        imwrite(f'{output_folder}/im_{idx}.png',im_i)
