...
# turn tensor of classes to one-hot encoding
...
# Sampling utility: Subclass of torch.Tensor
class Distribution(torch.Tensor):
    ...
    def sample_(self):
        if self.dist_type == 'normal':
            ...
            self.data = torch.from_numpy(data).type(self.type()).to(self.device)
...
# ----------------------------------------------------------------------
# ✅ FIXED: Proper top-level to_device()
# ----------------------------------------------------------------------
def to_device(data, device):
    """
    Move tensors, dicts, lists, or tuples to a given torch.device.
    """
    # normalize device
    if isinstance(device, list):
        device = torch.device(f"cuda:{device[0]}") if len(device) > 0 else torch.device("cpu")
