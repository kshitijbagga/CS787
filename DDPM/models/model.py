...
class TRGAN(nn.Module):
    ...
    def encode_style(self, simg, text=None):
        """
        Extract a style embedding vector from the style image (simg).
        This vector can be used as conditioning input for diffusion models.
        Args:
            simg (torch.Tensor): Style image tensor of shape (B, C, H, W)
            text (optional): Placeholder for future text conditioning
        Returns:
            torch.Tensor: Style embedding of shape (B, D)
        """
        import torch
        import torch.nn.functional as F
        device = simg.device
        netG = getattr(self, "netG", None)
        if netG is None:
            raise RuntimeError("TRGAN has no generator (netG) defined")

        # Find the first convolution layer (style feature extractor)
        first_conv = None
        ...
        if first_conv is None:
            raise RuntimeError("Could not find Conv2d layer in netG")

        # Forward pass through the first layer(s)
        with torch.no_grad():
            h = F.relu(first_conv(simg))
            # Global average pool -> style embedding
            emb = torch.mean(h, dim=[2, 3])  # shape (B, C)
        return emb
        ...
        for step,data in enumerate(dataloader): 
            ...
            for i in range(fake_images.shape[0]):
                for j in range(fake_images.shape[1]):
                    #imwrite(os.path.join(self.real_base, str(step*batch_size + i)+'_'+str(j)+'.png'), 255*(real_images[i,j])) 
                    imwrite(os.path.join(self.fake_base, str(step*self.batch_size + i)+'_'+str(j)+'.png'), 255*(fake_images[i,j])) 
        ...
        for step,data in enumerate(dataset_real): 
            ...
            for i in range(real_images.shape[0]):
                for j in range(real_images.shape[1]):
                    imwrite(os.path.join(self.real_base, str(step*self.batch_size + i)+'_'+str(j)+'.png'), 255*(real_images[i,j])) 
